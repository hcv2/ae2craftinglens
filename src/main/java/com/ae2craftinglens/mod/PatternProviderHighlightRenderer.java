...
@Mod.EventBusSubscriber(modid = AE2CraftingLens.MODID, value = Dist.CLIENT)
public class PatternProviderHighlightRenderer {
    ...
    private static RenderType getHighlightLines() {
        if (highlightLines == null) {
            highlightLines = RenderType.lines();
        }
    ...
    public static void onRenderWorld(RenderLevelStageEvent event) {
        ...
        var vertexConsumer = bufferSource.getBuffer(getHighlightLines());
    ...
    private static void renderHighlight(PoseStack poseStack, com.mojang.blaze3d.vertex.VertexConsumer consumer, BlockPos pos, float remainingSeconds) {
        if (!shouldBlink()) {
            return;
        }
    ...
    private static void renderBox(PoseStack poseStack, com.mojang.blaze3d.vertex.VertexConsumer consumer, AABB box, float r, float g, float b, float a) {
    ...
    private static void renderBox(com.mojang.blaze3d.vertex.VertexConsumer consumer, PoseStack poseStack, Vec3 topLeft, Vec3 bottomLeft, Vec3 topRight, Vec3 bottomRight, float r, float g, float b, float a) {
    ...
    private static void renderLine(com.mojang.blaze3d.vertex.VertexConsumer consumer, PoseStack poseStack, Vec3 from, Vec3 to, float r, float g, float b, float a) {
        Matrix4f mat = poseStack.last().pose();
        Vec3 normal = from.subtract(to);
        consumer.vertex(mat, (float) from.x, (float) from.y, (float) from.z).color(r, g, b, a).normal((float) normal.x, (float) normal.y, (float) normal.z).endVertex();
        consumer.vertex(mat, (float) to.x, (float) to.y, (float) to.z).color(r, g, b, a).normal((float) normal.x, (float) normal.y, (float) normal.z).endVertex();
    }
}
